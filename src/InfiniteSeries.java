public class InfiniteSeries {
    // A method that estimates e^x by adding the first n terms
    public static double myexp(double x, int n) {
        double sum = 1.0; // The first term is always 1
        double numerator = 1.0; // The numerator of the term
        double denominator = 1.0; // The denominator of the term
        for (int i = 1; i < n; i++) {
            numerator = numerator * x; // The numerator is the one before multiplied by x
            denominator = denominator * i; // The denominator is the one before multiplied by i
            sum = sum + numerator / denominator; // Add the term to the sum
        }
        return sum;
    }
